...
float genStar1(float r, float a, float s, float time) {
	float t = floor((time+0.5)/2.);
	float rnd1 = floor(random(t+1.297)*2.+0.5);
	float rnd2 = floor(random(t+2.378)*4.+0.5);
	
	float r1 = genSubStar(r,a,s,2.,0.05,0.5,8.);
	float r2 = genSubStar(r*3.,a+0.05+time/20.,s,3.+rnd1,0.3,1.,6.)*(1.-r);
	float r3 = genSubStar(r*2.,a+0.5+time/25.,s,3.+rnd1,0.2,1.,6.)*(1.-r)*s;
	float r4 = genSubStar(r*4.,a-time/5.,1.,6.+rnd2,0.4,1.,4.)*(1.-r)*s;
...
vec4 genStar(vec2 coord, float time, float worldTime, float day ) {
	...
	s = 1.-pow(1.-s,2.);
	float r0 = genStar1(r,a,s,worldTime);
	color = vec4(r0*tOpen);
