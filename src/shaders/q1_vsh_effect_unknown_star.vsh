...
void main() {
    ...
    // genStar Static Data
    {
		...
		float rnd2 = floor(random(t+0.127)*1.+0.5);
		float rnd3 = floor(random(t+1.397)*2.+0.5);

    	vec2 p = (vTextureCoord-0.5);
		float s = 1.-min(1., localTime*2.);
		s = 1.-pow(1.-s,2.);
		
		float a1 = (secTime*rnd1/50.)*PI2;
		float a2 = (0.05+secTime/20.)*PI2;
		float a3 = (0.5+secTime/25.)*PI2;
		float a4 = (-secTime/5.)*PI2;
		vStarConf[0] = vec2(p.x*cos(a1)-p.y*sin(a1), p.x*sin(a1)+p.y*cos(a1))/s+0.5;
		...
		if (rnd2 < 1.) {
			vStarMix[0] = vec3(1., 0., s);
		} else {
			vStarMix[0] = vec3(0., 1., s);
		}
