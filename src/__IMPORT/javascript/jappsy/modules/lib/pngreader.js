{
	...
	var PNGReader = function(bytes){
		if (typeof bytes == 'string'){
			bytes = Jappsy.binstring2buf(bytes);
			/*
			var bts = bytes;
	...
	PNGReader.prototype.decodePixels = function(callback){
		...
		try {
			var output = Jappsy.inflate(data);
			try {
				if (png.getInterlaceMethod() === 0){
					reader.interlaceNone(output);
				} else {
					reader.interlaceAdam7(output);
				}
			} catch (e){
				return callback(e);
			}
