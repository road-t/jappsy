...
vec4 genDark( vec2 coord, float time, float worldTime, float day ) {
	vec4 color = vec4(0.);
	...
	// Генерация затмения
	vec2 pDark = vec2(pCenter.x,pCenter.y);
	...
	float rLight = (1.-smoothstep(0.45,0.65,r))*rShadow;
	if (rLight > 0.) {
...
void main() {
	gl_FragColor = genDark(vTextureCoord, vTime[0], vTime[1], vTime[2]);
}
