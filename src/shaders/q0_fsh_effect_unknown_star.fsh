...
float genStar3(float r, float a, float s, float time) {
	...
	float r1 = genSubStar(r,a+time*rnd1/50.,s,6.,0.25,1.,6.);
	...
	float r5 = genSubStar(r*1.5,a,s,0.,1.,1.,1.)*0.5;
	return clamp(r1+r2+r3+r4+r5,0.,1.);
}
...
vec4 genStar(vec2 coord, float time, float worldTime, float day ) {
	...
	float s = 1.-min(1., time*2.);
	s = 1.-pow(1.-s,2.);
	float r0 = genStar3(r,a,s,worldTime);
	color = vec4(mix(vec3(1.5,1.,0.5),vec3(1.),r0*pow(1.-r,2.))*r0*s,r0*0.75*s);
